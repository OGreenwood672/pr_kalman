...
class KalmanFilter:
    def __init__(
        ...
        self.Q = None # Calculated each timestep due to varied dt
        
        # TODO: Work out
        # Measurement noise covariance (R) (uncertainty in corrected measurements)
        self.R = np.array([[1, 0],  # Uncertainty in the height measurement
                           [0, 1]])  # Uncertainty in the velocity measurement
        
        # Barometer measurement noise (this can be adjusted based on your sensor specs)
        self.R_barometer = np.array([[barometer_variance]])  # Barometer noise in height measurement
    ...
    def predict_with_accelerometer(self, acceleration, dt):
        ...
        self.Q = np.array([[0.5 * self.accelerometer_variance * (dt ** 2), 0], 
                           [0, self.accelerometer_variance * dt]])  # Assume some process noise (tuning parameter)
